...
@ActiveProfiles("test")
@Sql(
        scripts = "classpath:database/users/insert-task-test-users.sql",
        executionPhase = Sql.ExecutionPhase.BEFORE_TEST_METHOD
)
@Sql(
        scripts = "classpath:database/users/clean-task-test-data.sql",
...
class TaskControllerTest {
    ...
    private static final String ADMIN_EMAIL = "task-test-admin@gmail.com";
    private static final String MANAGER_EMAIL = "task-test-manager@gmail.com";
    private static final String MEMBER_EMAIL = "task-test-member@gmail.com";
    private static final String SECOND_MEMBER_EMAIL = "task-test-member-2@gmail.com";
    ...
    void createTask_ValidRequest_ReturnCreatedTask() throws Exception {
        ...
        mockMvc.perform(
                ...
                .andExpect(jsonPath("$.createdById").value(manager.getId()))
                .andExpect(jsonPath("$.name").value("Implement login page"))
                .andExpect(jsonPath("$.description").value("Build the login screen"))
        ...
        assertThat(savedTask.getName()).isEqualTo("Implement login page");
        assertThat(savedTask.getProject().getId()).isEqualTo(project.getId());
        assertThat(savedTask.getAssignee().getId()).isEqualTo(member.getId());
        assertThat(savedTask.getCreatedBy().getId()).isEqualTo(manager.getId());
    }
    ...
    void createTask_NonexistentProject_ReturnsNotFound() throws Exception {
        TaskCreateRequestDto requestDto = new TaskCreateRequestDto(
                999999L,
                null,
    ...
    void createTask_NonexistentAssignee_ReturnsNotFound() throws Exception {
        TaskCreateRequestDto requestDto = new TaskCreateRequestDto(
                project.getId(),
                999999L,
    ...
    void createTask_NonexistentLabel_ReturnsNotFound() throws Exception {
        TaskCreateRequestDto requestDto = new TaskCreateRequestDto(
                project.getId(),
                null,
    ...
    void getTaskById_ProjectMember_ReturnsTask() throws Exception {
        Task task = createTask(project, manager, member, "Fix bug", TaskStatus.IN_PROGRESS);
        ...
        mockMvc.perform(
                ...
                .andExpect(jsonPath("$.name").value("Fix bug"))
    ...
    void getTaskById_NonMember_ReturnsForbidden() throws Exception {
        Task task = createTask(project, manager, member, "Fix bug", TaskStatus.IN_PROGRESS);
    ...
    void getTasks_ProjectMember_ReturnsPage() throws Exception {
        createTask(project, manager, member, "Task One", TaskStatus.NOT_STARTED);
        createTask(project, manager, member, "Task Two", TaskStatus.NOT_STARTED);
    ...
    void getTasks_NonMember_ReturnsForbidden() throws Exception {
        createTask(project, manager, member, "Task One", TaskStatus.NOT_STARTED);
    ...
    void updateTask_ValidRequest_ReturnUpdatedTask() throws Exception {
        Task task = createTask(project, manager, member, "Old Name", TaskStatus.NOT_STARTED);
        ...
        mockMvc.perform(
                ...
                .andExpect(jsonPath("$.status").value("IN_PROGRESS"))
                .andExpect(jsonPath("$.assigneeId").value(secondMember.getId()));
        ...
        assertThat(updatedTask.getAssignee().getId()).isEqualTo(secondMember.getId());
    }

    @Test
    @DisplayName("Update task - team member - forbidden")
    void updateTask_TeamMember_ReturnsForbidden() throws Exception {
        Task task = createTask(project, manager, member, "Old Name", TaskStatus.NOT_STARTED);
        ...
        assertThat(unchangedTask.getName()).isEqualTo("Old Name");
    }
    ...
    void updateAssignedTask_Assignee_ReturnUpdatedTask() throws Exception {
        Task task = createTask(project, manager, member, "My Task", TaskStatus.NOT_STARTED);

        TaskAssignedUpdateRequestDto requestDto = new TaskAssignedUpdateRequestDto(
                "Updated by assignee",
        ...
        mockMvc.perform(
                        put("/api/tasks/{id}/assigned", task.getId())
                                .with(user(MEMBER_EMAIL).roles("TEAM_MEMBER"))
                                .contentType(APPLICATION_JSON)
                                .content(objectMapper.writeValueAsString(requestDto))
                )
                ...
                .andExpect(jsonPath("$.description").value("Updated by assignee"));
    ...
    void updateAssignedTask_NotAssignee_ReturnsForbidden() throws Exception {
        Task task = createTask(project, manager, member, "My Task", TaskStatus.NOT_STARTED);
        ...
        Task unchangedTask = taskRepository.findById(task.getId()).orElseThrow();
        assertThat(unchangedTask.getStatus()).isEqualTo(TaskStatus.NOT_STARTED);
    }

    @Test
    @DisplayName("Update assigned task - project manager role - forbidden")
    void updateAssignedTask_ProjectManagerRole_ReturnsForbidden() throws Exception {
        Task task = createTask(project, manager, member, "My Task", TaskStatus.NOT_STARTED);
    ...
    void deleteTask_ValidId_ReturnsNoContent() throws Exception {
        Task task = createTask(project, manager, member, "To Delete", TaskStatus.NOT_STARTED);
    ...
    void deleteTask_TeamMember_ReturnsForbidden() throws Exception {
        Task task = createTask(project, manager, member, "To Protect", TaskStatus.NOT_STARTED);
